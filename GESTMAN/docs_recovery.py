...
def get_filter_options(section):
    ...
    try:
        if section == 'compilazioni':
            ...
            # Template (solo da form_submissions + nome_voce da storico)
            ...
            # Tipo record (nuovo filtro)
            filters['tipo_record'] = ['form', 'esecuzione']
            ...
            c.execute("SELECT DISTINCT operatore_completamento FROM scadenze_calendario WHERE operatore_completamento IS NOT NULL ORDER BY operatore_completamento")
            filters['operatore_completamento'] = [row[0] for row in c.fetchall()]
...
def get_scadenze_docs():
    ...
    try:
        ...
        query = '''
            SELECT id, civico, asset, asset_tipo, data_scadenza, stato,
                   data_completamento, operatore_completamento,
                   note_completamento, created_at, updated_at
...
def bulk_delete_records(section):
    ...
    try:
        ...
        for record_id in record_ids:
            try:
                if section == 'compilazioni':
                    delete_compilazione(record_id)
                elif section == 'alert':
                    delete_alert(record_id)
                elif section == 'scadenze':
                    delete_scadenza(record_id)
                elif section == 'civici':
                    delete_civico(record_id)
                elif section == 'asset-types':
                    delete_asset_type(record_id)
                elif section == 'assets-inventory':
                    delete_asset_inventory(record_id)
                deleted_count += 1
            except Exception as e:
...
def format_column_name(column_name):
    ...
    column_map = {
        ...
        'civico': 'Civico',
        'asset': 'Asset',
        'operatore_completamento': 'Operatore Completamento',
        ...
        'operatore_esecuzione': 'Operatore Esecuzione',
...
def print_report():
    ...
    try:
        ...
        if section == 'asset-types':
            ...
            # Alert dal database gestman.db
            conn = get_gestman_connection()
            c = conn.cursor()
            c.execute('''
                SELECT id, tipo, titolo, descrizione, data_creazione,
                       operatore, civico_numero, asset_id, stato, note
                FROM alert
