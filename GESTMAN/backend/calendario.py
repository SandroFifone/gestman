...
def completa_scadenza(scadenza_id):
    ...
    try:
        ...
        if is_gruppo:
            ...
            # Calcola anche la scadenza ancora successiva per il campo data_prossima_scadenza
            if frequenza_tipo and frequenza_mesi:
                if HAS_DATEUTIL:
                    if frequenza_mesi < 1:
                        giorni = int(frequenza_mesi * 30)
                        scadenza_dopo = data_successiva + datetime.timedelta(days=giorni)
                    else:
                        scadenza_dopo = data_successiva + relativedelta(months=int(frequenza_mesi))
                else:
                    giorni_da_aggiungere = int(frequenza_mesi * 30)
                    scadenza_dopo = data_successiva + datetime.timedelta(days=giorni_da_aggiungere)
            else:
                scadenza_dopo = data_successiva + datetime.timedelta(days=30)
            ...
            """, (
                ...
                civico, asset, asset_tipo,
                data_successiva.isoformat(), scadenza_dopo.isoformat(),
